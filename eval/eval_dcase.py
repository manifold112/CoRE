...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Evaluate CoRE on DCASE-style multiple-choice AQA.")
    parser.add_argument("--config", type=str, default=None)

    parser.add_argument("--dataset_path", type=str, required=False)
    parser.add_argument("--audio_root", type=str, default=None)
    parser.add_argument("--output_dir", type=str, required=False, default="outputs/dcase")
...
def resolve_subset(example: dict[str, Any]) -> str:
    for key in ["subset", "task", "category", "split_name"]:
        if key in example:
            return str(example[key])
    return "ALL"
...
def summarize_by_permutation(records: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    subset_names = sorted({str(r["subset"]) for r in records})
    subset_accs_by_perm: dict[str, list[float]] = defaultdict(list)
    ...
    for perm_idx in sorted(by_perm.keys()):
        ...
        for subset in subset_names:
            subset_items = [x for x in items if str(x["subset"]) == subset]
            subset_correct = sum(int(x["correct"]) for x in subset_items)
            subset_total = len(subset_items)
            subset_acc = subset_correct / subset_total if subset_total > 0 else 0.0
            subset_metrics[subset] = {
                "accuracy": subset_acc,
                "correct": subset_correct,
                "total": subset_total,
            }
            subset_accs_by_perm[subset].append(subset_acc)
    ...
    subset_summary = {}
    for subset, vals in subset_accs_by_perm.items():
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    run_name = args.run_name or f"dcase_{args.model_type}_{args.method}"
    ...
    print(
        f"\n[DCASE] {args.model_type} + {args.method}: "
        f"{overall_summary['accuracy_mean'] * 100:.2f} ± {overall_summary['accuracy_std'] * 100:.2f}"
    )
    for subset, stats in overall_summary["subsets"].items():
        print(f"  - {subset}: {stats['accuracy_mean'] * 100:.2f} ± {stats['accuracy_std'] * 100:.2f}")
